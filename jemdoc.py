...
def dashlist1(f, ordered=False):
  ...
  for i in range(level):
    strOut += '</li></%s>' % ul
  strOut = strOut.replace('\n','') # remove all \n characters, cause issues with javascript
  strOut = strOut.replace('\'','\\\'') # replace ' with \' due to javascript issues
  return strOut
...
def procfile(f):
  ...
  while 1: # wait for EOF.
    ...
    # look for comments.
    ...
    elif p == '{' and colorGrid:    
      l = allreplace(nl(f,False,True)) # use nl without count and with code line flag
      r = re.compile(r'(?<!\\){(.*?)(?<!\\)}', re.M + re.S)
      g = re.findall(r, l)

      if len(g)>=4 and g[1] == 'colorGrid':
        ...
        out(f.outf, 'case %d:\n'%(numColorBlock+1))
        out(f.outf, 'var ss = "";\n')
        out(f.outf, 'ss += \'<td class="innerBlock">\';\n')
        out(f.outf, 'ss += \'<table class="colorBlock">\';\n')
        if len(g[8])>0:
          if g[2] == 'part':
            out(f.outf, 'ss += \'<tr><td class="cbHeading">\';\n')
          elif g[2] == 'full':
            out(f.outf, 'ss += \'<tr><td class="cbHeading" style="text-align:left;">\';\n')
          if g[9] == '':
            out(f.outf, 'ss += \'%s</td></tr>\';\n'%(g[8]))
          elif len(g[9])>0:
            out(f.outf, 'ss += \'<a class="bh" href="%s">%s</a></td></tr>\';\n'%(g[9],g[8]))
        if len(g[3])>0:
          out(f.outf, 'ss += \'<tr><td class="image">\';\n')
          if len(g[4])>0 and g[4][:5]=='round':
              if g[4][6:].isdigit():
                out(f.outf, 'ss += \'<img class="round" src="%s" alt="%s" style="width:%s;height:%s;border-radius:%d%%;" /></td></tr>\';\n'%(g[3],g[5],g[6],g[7],int(g[4][6:])))
              else:
                out(f.outf, 'ss += \'<img class="round" src="%s" alt="%s" style="width:%s;height:%s;" /></td></tr>\';\n'%(g[3],g[5],g[6],g[7]))
          else:
            out(f.outf, 'ss += \'<img src="%s" alt="%s" style="width:%s;height:%s;" /></td></tr>\';\n'%(g[3],g[5],g[6],g[7]))
        if len(g[10])>0:
          out(f.outf, 'ss += \'<tr><td class="bottomText">\';\n')
          out(f.outf, 'ss += \'%s\';\n'%(g[10]))
          out(f.outf, 'ss += \'</td></tr>\';\n')  
        listFill = False
        while(1):
          ...
          if p == '\n':
            ...
            break
        out(f.outf, 'ss += \'</table></td>\';\n')
        out(f.outf, 'var out = {str:ss, width:%s, type:\'%s\'};\n'%(g[6][:-2],g[2]))
        out(f.outf, 'break;\n')
        numColorBlock += 1
      ...
      if colorGrid:
        out(f.outf, 'default:\n')
        out(f.outf, 'out = {str:\'empty\', width:0};\n')
        out(f.outf, '}\n')
        out(f.outf, 'return out;\n')
        out(f.outf, '}\n')
        out(f.outf, '</script>\n')
        colorGrid = False
        numColorBlock = 0
        ...
        if len(g)>=4 and g[1] == 'colorGrid':
          ...
          if not colorGridDefault:
            # create default blocks
            out(f.outf, '<table class="grid" id="projTable%d"><tr>\n'%(numGridTable))
            out(f.outf,'</tr></table>\n')
            out(f.outf, '<script>\n')
            out(f.outf, 'gridTableInit%d();\n'%(numGridTable))
            out(f.outf, 'window.addEventListener("resize", gridTableInit%d);\n'%(numGridTable))
            out(f.outf, 'function gridTableInit%d(){\n'%(numGridTable))
            out(f.outf, 'var projCount = 0;\n')
            out(f.outf, 'var varWidth = 150;\n')
            out(f.outf, 'var table = document.getElementById("projTable%d");\n'%(numGridTable))
            out(f.outf, 'table.innerHTML = "";\n')
            out(f.outf, 'var rowId = 0;\n')
            out(f.outf, 'var consecutiveBreaks = 0;\n')
            out(f.outf, 'while(1){\n')
            out(f.outf, 'out = projElem%d(projCount+1);\n'%(numGridTable))
            out(f.outf, 'if (out.str == \'empty\'){\n')
            out(f.outf, 'break;\n')
            out(f.outf, '}\n')
            out(f.outf, 'var row = table.insertRow(rowId);\n')
            out(f.outf, 'var cell = row.insertCell(0);\n')
            out(f.outf, 'var x = document.createElement(\'TABLE\');\n')
            out(f.outf, 'x.className = \'grid\';\n')
            out(f.outf, 'var tr = document.createElement(\'tr\');\n')
            out(f.outf, 'var rowBegin = false;\n')
            out(f.outf, 'while(1){\n')
            out(f.outf, 'var out = projElem%d(projCount+1);\n'%(numGridTable))
            out(f.outf, 'if (out.str == \'empty\'){\n')
            out(f.outf, 'consecutiveBreaks = 0;\n')
            out(f.outf, 'break;\n')
            out(f.outf, '}\n')
            out(f.outf, 'if (varWidth + out.width> window.outerWidth){\n')
            out(f.outf, 'consecutiveBreaks += 1;\n')
            out(f.outf, 'break;\n')
            out(f.outf, '}\n')
            out(f.outf, 'var td = document.createElement(\'td\');\n')
            out(f.outf, 'if (out.type == \'part\'){\n')
            out(f.outf, 'td.className = \'topBox\';\n')
            out(f.outf, 'var widthVar = out.width.toString() + \'px\';\n')
            out(f.outf, 'td.width = widthVar;\n')
            out(f.outf, 'td.innerHTML = out.str;\n')
            out(f.outf, 'tr.appendChild(td);\n')
            out(f.outf, 'varWidth += out.width;\n')
            out(f.outf, 'varWidth += 20;\n')
            out(f.outf, 'projCount += 1;\n')
            out(f.outf, 'consecutiveBreaks = 0;\n')
            out(f.outf, 'rowBegin = true;\n')
            out(f.outf, 'continue;\n')
            out(f.outf, '}\n')
            out(f.outf, 'else if(out.type == \'full\' && !rowBegin){\n')
            out(f.outf, 'td.className = \'fullBlock\';\n')
            out(f.outf, 'td.innerHTML = out.str;\n')
            out(f.outf, 'tr.appendChild(td);\n')
            out(f.outf, 'projCount += 1;\n')
            out(f.outf, 'consecutiveBreaks = 0;\n')
            ...
            out(f.outf, '}\n')
            out(f.outf, '}\n')
            out(f.outf, 'if (consecutiveBreaks>1){\n')
            out(f.outf, 'break;\n')
            out(f.outf, '}\n')
            out(f.outf, 'x.appendChild(tr)\n')
            out(f.outf, 'cell.appendChild(x);\n')
            out(f.outf, 'varWidth = 150;\n')
            out(f.outf, 'rowId += 1;\n')
            out(f.outf, '}\n')
            out(f.outf, '}\n')
            out(f.outf, 'function projElem%d(ind){\n'%(numGridTable))
            out(f.outf, 'switch(ind){\n')
            colorGridDefault = True
        # handles
    	  # {}{colorBlock}{type}{img_source}{img_layout}{alttext}{img_width}{img_height}{title}{linktarget}{caption}.
          g += ['']*(11 - len(g))
          ...
          # out(f.outf, '<td><table class="colorBlock"><tr><td align="center">\n')
          # if g[6]:
          ...
          # if g[6]:
          #   out(f.outf, '</a>')
          # out(f.outf, '&nbsp;</td></tr><tr><td align="left">\n')
          colorGrid = True
